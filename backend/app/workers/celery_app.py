...
celery = Celery("devops_review", broker=settings.redis_url, backend=settings.redis_url) if Celery else None

if celery:
    @celery.task(name="analyze_pull_request")
    def analyze_pull_request(pull_request_id: int, trigger_event: str = "queued") -> int:
