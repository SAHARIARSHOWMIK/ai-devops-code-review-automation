...
def deduplicate(findings: list[dict]) -> list[dict]:
    ...
    for finding in findings:
        ...
        finding["fingerprint"] = finding.get("fingerprint") or fingerprint_finding(
            finding
        )
        match = next(
            (x for x in merged if x["fingerprint"] == finding["fingerprint"]), None
        )
        if not match:
            ...
            match = next(
                (
                    x
                    for x in merged
                    if x.get("file_path") == finding.get("file_path")
                    and x.get("category") == finding.get("category")
                    and SequenceMatcher(
                        None, x.get("title", ""), finding.get("title", "")
                    ).ratio()
                    > 0.88
                ),
                None,
        ...
        if match:
            ...
            match["confidence"] = min(
                0.99,
                max(match.get("confidence", 0.7), finding.get("confidence", 0.7))
                + 0.05,
            )
            if len(finding.get("evidence", "")) > len(match.get("evidence", "")):
                match["evidence"] = finding["evidence"]
        else:
            finding["sources"] = finding.get("sources") or [
