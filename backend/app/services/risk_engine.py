...
def calculate_risk(
    pr, findings: Iterable, test_recommendations: Iterable | None = None
) -> RiskResult:
    score = 8
    ...
    if total_changes > 1000:
        score += 20
        factors.append({"factor": "Very large code change", "points": 20})
    elif total_changes > 400:
        score += 12
        factors.append({"factor": "Large code change", "points": 12})
    elif total_changes < 50:
        score -= 3
        factors.append({"factor": "Small isolated change", "points": -3})
    ...
    for label, terms in sensitive_terms.items():
        if any(term in diff for term in terms):
            ...
            score += points
            factors.append({"factor": label.capitalize(), "points": points})
    ...
    for severity, count in severity_counts.items():
        ...
        if points:
            score += points
            factors.append(
                {"factor": f"{count} {severity} finding(s)", "points": points}
            )
    ...
    if recommendations:
        high = sum(
            ...
            if (item.priority if hasattr(item, "priority") else item.get("priority"))
            == "high"
        )
        points = min(5 + high * 3, 14)
        score += points
        factors.append({"factor": "Missing or recommended tests", "points": points})

    if pr.changed_files and all(
        str(f.get("filename", "")).lower().endswith((".md", ".txt"))
        for f in pr.changed_files
    ):
        score -= 10
        factors.append({"factor": "Documentation-only change", "points": -10})
    ...
    return RiskResult(
        score=score, level=level, factors=factors, recommendation=recommendation
    )
