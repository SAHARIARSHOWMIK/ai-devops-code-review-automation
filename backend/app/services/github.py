...
class GitHubAppClient:
    ...
    def configured(self) -> bool:
        return bool(
            ...
            and self.settings.github_private_key_path
        )
    ...
    def fetch_files(self, owner: str, repo: str, number: int) -> list[dict]:
        response = httpx.get(
            f"{self.settings.github_api_url}/repos/{owner}/{repo}/pulls/{number}/files",
            headers=self._headers(),
            timeout=30,
        )
        response.raise_for_status()
    ...
    def fetch_commits(self, owner: str, repo: str, number: int) -> list[dict]:
        response = httpx.get(
            f"{self.settings.github_api_url}/repos/{owner}/{repo}/pulls/{number}/commits",
            headers=self._headers(),
            timeout=30,
        )
        response.raise_for_status()
    ...
    def download_archive(self, owner: str, repo: str, ref: str) -> bytes:
        response = httpx.get(
            f"{self.settings.github_api_url}/repos/{owner}/{repo}/tarball/{ref}",
            headers=self._headers(),
            timeout=120,
    ...
    def publish_review(self, owner: str, repo: str, number: int, payload: dict) -> dict:
        response = httpx.post(
            f"{self.settings.github_api_url}/repos/{owner}/{repo}/pulls/{number}/reviews",
            headers=self._headers(),
            json=payload,
            timeout=30,
        )
