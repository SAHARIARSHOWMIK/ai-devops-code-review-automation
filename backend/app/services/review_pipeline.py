...
from ..models import AnalyzerRun, DocumentationRecommendation, Finding, PullRequest, ReviewRun, TestRecommendation
...
def execute_review(db: Session, pull_request_id: int, trigger_event: str = "manual") -> ReviewRun:
    pr = db.query(PullRequest).filter(PullRequest.id == pull_request_id).first()
    if not pr:
        raise ValueError("Pull request not found")
    previous_runs = db.query(ReviewRun).filter(ReviewRun.pull_request_id == pr.id).order_by(ReviewRun.version.desc()).all()
    ...
    try:
        ...
        if not settings.demo_mode and GitHubAppClient().configured:
            try:
                with RepositoryContextCollector().workspace(pr.repository.owner, pr.repository.name, pr.current_commit_sha) as workspace:
                    analyzer_results = run_language_analyzers(language, pr.diff_text or "", pr.changed_files or [], workspace=workspace)
            except Exception as exc:
                analyzer_results = run_language_analyzers(language, pr.diff_text or "", pr.changed_files or [])
        ...
        if policy and (pr.additions + pr.deletions) > policy.maximum_diff_size:
            context["diff_text"] = (context["diff_text"] or "")[:get_settings().max_diff_characters]
            context["analysis_note"] = "Diff exceeded repository policy and was truncated for bounded analysis."
        ai_output = review_context(context)
        ai_findings = [item.model_dump() for item in ai_output.findings if item.confidence >= 0.65]
        if policy and not policy.security_checks:
            raw_findings = [item for item in raw_findings if item.get("category") != "security"]
            ai_findings = [item for item in ai_findings if item.get("category") != "security"]
        raw_findings.extend(ai_findings)
        normalized = deduplicate(raw_findings)

        previous_fingerprints = {f.fingerprint: f for old in previous_runs[:1] for f in old.findings}
        ...
        for old_fp, old_finding in previous_fingerprints.items():
            if old_finding.status not in {"dismissed", "false_positive", "suppressed"}:
                old_finding.status = "still_present" if old_fp in new_fingerprints else "fixed"
        ...
        for item in (ai_output.test_recommendations if not policy or policy.test_review else []):
            row = TestRecommendation(review_run_id=run.id, **item)
            db.add(row)
            test_rows.append(row)
        for item in (ai_output.documentation_recommendations if not policy or policy.documentation_review else []):
        ...
        db.rollback()
        run = db.query(ReviewRun).filter(ReviewRun.id == run.id).first()
        pr = db.query(PullRequest).filter(PullRequest.id == pull_request_id).first()
        if run:
            run.status = "failed"
