...
def publish_review(
    ...
    inline = [
        f
        for f in approved
        if f.publish_recommendation == "inline" and f.status != "internal_note"
    ]
    body_parts: list[str] = []
    if include_summary:
        body_parts.append(
    ...
    publication = GitHubPublication(
        review_run_id=run.id, comment_type="review", status="pending", payload=payload
    )
    db.add(publication)
    db.flush()
    ...
    try:
        if dry_run or settings.demo_mode:
            ...
            pr = run.pull_request
            repo = pr.repository
            result = GitHubAppClient().publish_review(
                repo.owner, repo.name, pr.github_number, payload
            )
