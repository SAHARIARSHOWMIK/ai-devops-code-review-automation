...
def login(payload: LoginRequest, db: Session = Depends(get_db)) -> dict:
    ...
    return {
        "access_token": create_access_token(
            user.email, user.role, user.organization_id
        ),
        # OAuth token type, not a password.
        "token_type": "bearer",
        "user": serialize(user),
    }
...
def current_organization(
    ...
    return serialize(organization_or_404(db, user.organization_id))
...
def list_repositories(
    ...
    if language:
        query = query.filter(
            func.lower(Repository.primary_language) == language.lower()
        )
    if status_filter:
...
def create_repository(
    ...
    user: User = Depends(
        require_roles(
            ...
            UserRole.REPOSITORY_MAINTAINER.value,
        )
    ),
    db: Session = Depends(get_db),
) -> dict:
    ...
    db.add(
        RepositoryPolicy(
            repository_id=repo.id, review_profile=payload.active_review_profile
        )
    )
    record_audit(
        db,
...
def repository_detail(
    ...
    data["pull_requests"] = [
        serialize(pr)
        for pr in db.query(PullRequest)
        .filter(PullRequest.repository_id == repo.id)
    ...
    category_counts = Counter(
        ...
        for run in pr.review_runs
        for f in run.findings
    )
    data["recurring_categories"] = [
...
def list_suppressions(
    ...
    return [
        serialize(x)
        for x in db.query(SuppressionRule)
        .filter(SuppressionRule.repository_id == repo.id)
        .all()
    ]
...
def create_suppression(
    ...
    repo = repository_or_404(db, repository_id, user)
    row = SuppressionRule(
        repository_id=repo.id, created_by=user.id, **payload.model_dump()
    )
    db.add(row)
    record_audit(
        db,
...
def list_pull_requests(
    ...
    query = (
        db.query(PullRequest)
        .join(Repository)
        .filter(Repository.organization_id == user.organization_id)
    )
    if risk_level:
...
def create_pull_request(
    ...
    record_audit(
        ...
        new_value={"number": row.github_number, "title": row.title},
    )
    db.commit()
    db.refresh(row)
    return serialize(row)
...
def pull_request_detail(
    pr_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)
) -> dict:
    pr = pr_or_404(db, pr_id, user)
    data = serialize(pr)
    data["repository"] = {
        "id": pr.repository.id,
        "full_name": f"{pr.repository.owner}/{pr.repository.name}",
        "language": pr.repository.primary_language,
        "profile": pr.repository.active_review_profile,
    }
    latest = (
        db.query(ReviewRun)
...
def analyze_pull_request(
    ...
    record_audit(
        ...
        new_value={"trigger": payload.trigger_event},
    )
    db.commit()
        ...
        data.update(
            {
                ...
                "published_findings": sum(
                    f.status == "published" for f in row.findings
                ),
...
def review_detail_payload(db: Session, run: ReviewRun) -> dict:
    ...
    data["findings"] = [
        ...
        .filter(Finding.review_run_id == run.id)
        .order_by(Finding.severity, Finding.file_path)
        .all()
    ]
    data["analyzer_runs"] = [
        serialize(x)
        for x in db.query(AnalyzerRun).filter(AnalyzerRun.review_run_id == run.id).all()
    ]
    data["test_recommendations"] = [
        serialize(x)
        for x in db.query(TestRecommendation)
        .filter(TestRecommendation.review_run_id == run.id)
        .all()
    ]
    data["documentation_recommendations"] = [
        serialize(x)
        for x in db.query(DocumentationRecommendation)
        .filter(DocumentationRecommendation.review_run_id == run.id)
        .all()
    ]
    data["publications"] = [
        serialize(x)
        for x in db.query(GitHubPublication)
        .filter(GitHubPublication.review_run_id == run.id)
        .all()
    ]
    return data
...
def approve_all(
    ...
    for finding in run.findings:
        if finding.status in {"awaiting_review", "new", "still_present"}:
            ...
            db.add(
                FindingDecision(
                    finding_id=finding.id, reviewer_id=user.id, decision="approve"
                )
            )
    run.status = "approved_for_publication"
    run.pull_request.review_status = "approved_for_publication"
    record_audit(
        db,
        "review.approved_all",
        actor=user,
        repository_id=run.pull_request.repository_id,
        pull_request_id=run.pull_request_id,
        new_value={"findings": updated},
    )
    db.commit()
...
def decide_finding(
    ...
    ensure_same_organization(
        finding.review_run.pull_request.repository.organization_id, user
    )
    status_map = {
        "approve": "approved",
        "edit": "edited",
    ...
    if payload.create_suppression or payload.decision == "suppress":
        db.add(
            SuppressionRule(
                repository_id=finding.review_run.pull_request.repository_id,
                rule_type="fingerprint",
                rule_identifier=finding.fingerprint,
                file_pattern=finding.file_path,
                reason=payload.comment or "Created from finding decision",
                created_by=user.id,
            )
        )
    record_audit(
        db,
        f"finding.{payload.decision}",
        actor=user,
        repository_id=finding.review_run.pull_request.repository_id,
        pull_request_id=finding.review_run.pull_request_id,
        old_value=old,
        new_value={"status": finding.status, "comment": payload.comment},
    )
    db.commit()
    db.refresh(finding)
...
def publish(
    ...
    record_audit(
        ...
        repository_id=run.pull_request.repository_id,
        pull_request_id=run.pull_request_id,
        new_value={"dry_run": payload.dry_run, "decision": payload.review_decision},
    )
    db.commit()
    return {
        "published": len(publications),
        "items": [serialize(x) for x in publications],
    }
...
def pending_approvals(
    ...
    return [
        {
            **serialize(r),
            "repository": f"{r.pull_request.repository.owner}/{r.pull_request.repository.name}",
            "pull_request_number": r.pull_request.github_number,
            "pull_request_title": r.pull_request.title,
            "risk_level": r.pull_request.risk_level,
            "total_findings": len(r.findings),
            "high_critical": sum(
                f.severity in {"high", "critical"} for f in r.findings
            ),
            "assigned_reviewer": r.pull_request.assigned_reviewer,
        }
        for r in rows
    ]
...
def analytics_overview(
    ...
    prs = (
        db.query(PullRequest).filter(PullRequest.repository_id.in_(repo_ids)).all()
        if repo_ids
        else []
    )
    ...
    return {
        ...
        "open_pull_requests": sum(pr.state == "open" for pr in prs),
        ...
        "high_risk_pull_requests": sum(
            pr.risk_level in {"high", "critical"} for pr in prs
        ),
        "critical_findings": sum(
            f.severity == "critical"
            and f.status not in {"fixed", "dismissed", "false_positive"}
            for f in findings
        ),
        "published_reviews": sum(r.status == "published" for r in runs),
        "failed_analysis_jobs": sum(r.status == "failed" for r in runs),
        "average_review_duration_ms": round(sum(durations) / len(durations))
        if durations
        else 0,
        "most_common_category": categories.most_common(1)[0][0] if categories else None,
        "false_positive_rate": round(false_positive / decided * 100, 1)
        if decided
        else 0,
        ...
        "risk_distribution": [
            {"name": level.title(), "value": sum(pr.risk_level == level for pr in prs)}
...
def security_analytics(
    ...
    by_repo = Counter(
        f"{f.review_run.pull_request.repository.owner}/{f.review_run.pull_request.repository.name}"
        for f in findings
    )
    ...
    return {
        "total": len(findings),
        "unresolved": sum(
            f.status not in {"fixed", "dismissed", "false_positive", "suppressed"}
            for f in findings
        ),
        "by_repository": [{"name": k, "value": v} for k, v in by_repo.most_common()],
        "by_severity": [{"name": k, "value": v} for k, v in by_severity.items()],
        "recent": [serialize(f) for f in findings[-20:]],
    }
...
def quality_analytics(
    ...
    rejected = sum(
        f.status in {"dismissed", "false_positive", "suppressed"} for f in findings
    )
    return {
        "findings": len(findings),
        "accepted": accepted,
        "rejected": rejected,
        "accepted_rate": round(accepted / max(accepted + rejected, 1) * 100, 1),
        "false_positive_rate": round(
            sum(f.status == "false_positive" for f in findings)
            / max(accepted + rejected, 1)
            * 100,
            1,
        ),
        "by_category": [
            {"name": k, "value": v}
            for k, v in Counter(f.category for f in findings).most_common()
        ],
        "by_status": [
            {"name": k, "value": v}
            for k, v in Counter(f.status for f in findings).most_common()
        ],
    }
...
def github_status(
    ...
    org = organization_or_404(db, user.organization_id)
    client = GitHubAppClient()
    return {
        "configured": client.configured,
        "demo_mode": get_settings().demo_mode,
        "organization_installation_id": org.github_installation_id,
        # Compare against the documented development placeholder.
        "webhook_secret_configured": get_settings().github_webhook_secret
...
def retry_failed(
    ...
    user: User = Depends(
        require_roles(
            UserRole.PLATFORM_ADMIN.value, UserRole.REPOSITORY_MAINTAINER.value
        )
    ),
    db: Session = Depends(get_db),
) -> dict:
    run = (
        db.query(ReviewRun)
        .filter(ReviewRun.id == review_run_id, ReviewRun.status == "failed")
        .first()
    )
    if not run:
        raise HTTPException(404, "Failed review run not found")
    ensure_same_organization(run.pull_request.repository.organization_id, user)
...
async def github_webhook(
    ...
    delivery = (
        ...
        or f"manual-{int(datetime.now(timezone.utc).timestamp() * 1000)}"
    )
    event_name = request.headers.get("X-GitHub-Event", "unknown")
    if not verify_webhook_signature(body, signature):
        raise HTTPException(401, "Invalid webhook signature")
    existing = (
        db.query(WebhookEvent).filter(WebhookEvent.delivery_id == delivery).first()
    )
    if existing:
        return {
            "accepted": True,
    ...
    if (
        ...
        and action in {"opened", "reopened", "synchronize", "review_requested"}
        ...
        organization = (
            ...
            .filter(Organization.github_installation_id == installation_id)
            .first()
            if installation_id
            else None
        )
        ...
        if repo:
            ...
            pr = (
                ...
                .filter(
                    ...
                    PullRequest.github_number == number,
                )
                .first()
            )
            values = {
                "title": gh_pr.get("title", "Untitled pull request"),
                "description": gh_pr.get("body") or "",
                "author": (gh_pr.get("user") or {}).get("login", "unknown"),
                "base_branch": (gh_pr.get("base") or {}).get("ref", "main"),
                "head_branch": (gh_pr.get("head") or {}).get("ref", "feature"),
                "current_commit_sha": (gh_pr.get("head") or {}).get("sha", "unknown"),
                "changed_files_count": gh_pr.get("changed_files", 0),
            ...
            if not get_settings().demo_mode and GitHubAppClient().configured:
                try:
                    values.update(
                        RepositoryContextCollector().collect_pull_request(
                            owner,
                            name,
                            number,
                            repo.policy.ignored_paths if repo.policy else [],
                        )
                    )
                except Exception as exc:
                    row.error_details = (
                        f"Context collection failed; webhook metadata retained: {exc}"
                    )
            if not pr:
                pr = PullRequest(repository_id=repo.id, github_number=number, **values)
                db.add(pr)
            else:
            ...
            eligible = bool(
                policy
                and policy.review_enabled
                and values["base_branch"] in policy.monitored_branches
            )
            ...
            if eligible:
                ...
                row.error_details = (
                    "Pull request is outside the configured review policy"
                )
                db.commit()
        else:
            row.status = "ignored"
