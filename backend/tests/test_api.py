...
def test_login_failure(client):
    response = client.post("/api/auth/login", json={"email": "admin@demo.com", "password": "wrong-password"})
...
def test_policy_update(client, admin_headers):
    repo_id = client.get("/api/repositories", headers=admin_headers).json()[0]["id"]
    current = client.get(f"/api/repositories/{repo_id}/policy", headers=admin_headers).json()
    current["minimum_severity"] = "high"
    current.pop("id"); current.pop("repository_id")
    response = client.put(f"/api/repositories/{repo_id}/policy", headers=admin_headers, json=current)
    assert response.status_code == 200
...
def test_auditor_cannot_update_policy(client, auditor_headers, admin_headers):
    repo_id = client.get("/api/repositories", headers=admin_headers).json()[0]["id"]
    payload = {
        "review_enabled": True, "review_profile": "standard_application", "monitored_branches": ["main"],
        "ignored_paths": [], "analyzer_settings": {}, "maximum_diff_size": 5000, "minimum_severity": "medium",
        "security_checks": True, "test_review": True, "documentation_review": True, "approval_required": True,
        "auto_post_summary": False, "re_review_on_push": True,
    }
    assert client.put(f"/api/repositories/{repo_id}/policy", headers=auditor_headers, json=payload).status_code == 403


def test_create_repository(client, admin_headers):
    response = client.post("/api/repositories", headers=admin_headers, json={"owner": "acme", "name": "ml-risk-service", "primary_language": "Python", "active_review_profile": "data_ml_project"})
    assert response.status_code == 201
    assert response.json()["name"] == "ml-risk-service"


def test_duplicate_repository_rejected(client, admin_headers):
    payload = {"owner": "acme", "name": "duplicate-service", "primary_language": "Python"}
    assert client.post("/api/repositories", headers=admin_headers, json=payload).status_code == 201
    assert client.post("/api/repositories", headers=admin_headers, json=payload).status_code == 409
...
def test_risk_filter(client, admin_headers):
    response = client.get("/api/pull-requests?risk_level=critical", headers=admin_headers)
    assert response.status_code == 200
...
def test_manual_reanalysis(client, admin_headers):
    pr_id = client.get("/api/pull-requests", headers=admin_headers).json()[0]["id"]
    response = client.post(f"/api/pull-requests/{pr_id}/analyze", headers=admin_headers, json={"trigger_event": "manual.test", "force": True})
    assert response.status_code == 200
...
def test_review_history(client, admin_headers):
    pr_id = client.get("/api/pull-requests", headers=admin_headers).json()[0]["id"]
    rows = client.get(f"/api/pull-requests/{pr_id}/review-history", headers=admin_headers).json()
    assert len(rows) >= 1
...
def test_finding_approve(client, admin_headers):
    ...
    response = client.post(f"/api/findings/{finding['id']}/decision", headers=admin_headers, json={"decision": "approve", "comment": "Valid finding"})
    assert response.status_code == 200
...
def test_finding_edit(client, admin_headers):
    ...
    finding = client.get(f"/api/pull-requests/{pr['id']}", headers=admin_headers).json()["latest_review"]["findings"][0]
    response = client.post(f"/api/findings/{finding['id']}/decision", headers=admin_headers, json={"decision": "edit", "edited_content": "Validate and authorize the vendor document replacement.", "comment": "Clarified wording"})
    assert response.status_code == 200
...
def test_false_positive_decision(client, admin_headers):
    ...
    finding = client.get(f"/api/pull-requests/{pr['id']}", headers=admin_headers).json()["latest_review"]["findings"][0]
    response = client.post(f"/api/findings/{finding['id']}/decision", headers=admin_headers, json={"decision": "false_positive", "comment": "Validated by shared middleware"})
...
def test_suppression_creation_from_finding(client, admin_headers):
    ...
    response = client.post(f"/api/findings/{finding['id']}/decision", headers=admin_headers, json={"decision": "suppress", "comment": "Accepted generated-code exception", "create_suppression": True})
    assert response.status_code == 200
    repo_id = detail["repository"]["id"]
    assert client.get(f"/api/repositories/{repo_id}/suppressions", headers=admin_headers).json()
...
def test_approve_all_and_publish_dry_run(client, admin_headers):
    ...
    response = client.post(f"/api/review-runs/{run_id}/approve-all", headers=admin_headers)
    assert response.status_code == 200
    publish = client.post(f"/api/review-runs/{run_id}/publish", headers=admin_headers, json={"review_decision": "request_changes", "include_summary": True, "dry_run": True})
...
def test_auditor_cannot_publish(client, auditor_headers, admin_headers):
    pr = client.get("/api/pull-requests", headers=admin_headers).json()[0]
    run_id = client.get(f"/api/pull-requests/{pr['id']}", headers=admin_headers).json()["latest_review"]["id"]
    response = client.post(f"/api/review-runs/{run_id}/publish", headers=auditor_headers, json={"dry_run": True})
...
def test_notifications(client, admin_headers):
    ...
    if rows:
        response = client.post(f"/api/notifications/{rows[0]['id']}/read", headers=admin_headers)
...
def webhook_headers(body: bytes, delivery: str):
    ...
    return {"X-Hub-Signature-256": f"sha256={signature}", "X-GitHub-Delivery": delivery, "X-GitHub-Event": "ping", "Content-Type": "application/json"}


def test_valid_webhook(client):
    body = json.dumps({"zen": "Keep it logically awesome"}).encode()
    response = client.post("/api/webhooks/github", content=body, headers=webhook_headers(body, "test-delivery-valid"))
    assert response.status_code == 200
    assert response.json()["accepted"] is True


def test_invalid_webhook_rejected(client):
    response = client.post("/api/webhooks/github", content=b"{}", headers={"X-Hub-Signature-256": "sha256=invalid", "X-GitHub-Delivery": "invalid-delivery", "Content-Type": "application/json"})
    assert response.status_code == 401
