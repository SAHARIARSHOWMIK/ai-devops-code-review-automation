...
def test_risk_score_bounded():
    pr = SimpleNamespace(
        additions=5000,
        deletions=5000,
        diff_text="auth migration payment package.json",
...
def test_deterministic_secret_detection():
    findings = deterministic_findings(
        '+API_KEY = "secret-value-123"', [{"filename": "settings.py"}]
    )
    assert findings
...
def test_analyzer_adapter_returns_language_tools():
    results = run_language_analyzers(
        "Python", "+eval(user_input)", [{"filename": "app.py"}]
    )
    ...
    output = mock_review(
        {
            ...
            "diff_text": "+payload = await request.json()",
        }
    )
...
def test_mock_ai_recommends_tests_when_missing():
    output = mock_review(
        {
            "github_number": 1,
            "changed_files": [{"filename": "service.py"}],
            "diff_text": "+db.commit()",
        }
    )
...
def test_mock_ai_skips_test_recommendation_when_test_changed():
    output = mock_review(
        {
            "github_number": 1,
            "changed_files": [{"filename": "tests/test_service.py"}],
